  ...
  <div id="project-create-page">
    ...
    <v-form v-else ref="form" v-model="valid" class="mt-4">
      <v-container>
        ...
        <div v-for="(contact, i) in form.period" :key="i">
          <div style="font-size: 18px"><b>งวดที่ {{ i + 1 }}</b></div>
          <v-row>
            <v-col :cols="12" :md="6">
              <!-- <v-text-field v-model="form[`period${i}`]" label="รายละเอียดรายการวัสดุ/ครุภัณฑ์"/> -->
              <v-text-field v-model="contact.period" label="รายละเอียดรายการวัสดุ/ครุภัณฑ์"/>
            </v-col>
            <v-col :cols="12" :md="3">
              <!-- <InputDatePicker :value.sync="form[`period${i}Date`]" label="วันที่ส่งมอบ"/> -->
              <InputDatePicker :value.sync="contact.periodDate" label="วันที่ส่งมอบ"/>
            </v-col>
            <v-btn v-if="form.period.length > 1" icon @click="removePeriod(i)">
              <i class="material-icons">delete</i>
            </v-btn>
          </v-row>
        </div>
        <v-row class="mt-4">
          <v-btn block rounded outlined @click="addPeriod">เพิ่มงวด</v-btn>
        </v-row>
        <!-- </div> -->
      </v-container>
...
<script>
  export default {
    ...
    mounted () {
      ...
      else this.addCommittee(), this.addPeriod()
    },
    methods: {
      async getData () {
        try {
      ...
      addPeriod () {
        ...
        this.form.period = [ ...this.form.period, newContact ]
      },
      removePeriod (i) {
        this.form.period.splice(i, 1)
      },
      onEditProject () {
