      ...
      <v-card-text class="black--text pt-2">
        <div>
          <v-form ref="form" v-model="valid" lazyValidation>
            <SelectDropdown v-if="tabIndex > 0" :value.sync="form.majorCategoryId" label="หมวดหมู่พัสดุ *" apiPath="equipment/category/getMejorCategorys" :items="majorCategoryItems" :rules="categoryRule" required :disabled="isEdit" @select="onChangeCategory"/>
